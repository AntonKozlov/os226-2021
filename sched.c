...
int sys_exit(int code) {
    for (int i = 0; i < FD_MAX; i++) { /* перебираем все файловые дискрипторы */
        if (current->fd[i] != NULL) { /* проверяем, что файловый дискриптор использовался */
            struct pipe *p = fd2pipe(i, NULL); /* получаем pipe из ФД */
            if (current->fd[i]->ops->read == pipe_read)
                p->rdclose = 1; /* если он был открыт для чтения, то закрываем */
            else if (current->fd[i]->ops->write == pipe_write) p->wrclose = 1; /* аналогично с записью */
            sys_close(i); /* закрываем ФД */
        }
    }
    doswitch();
}
...
static int pipe_read(int fd, void *buf, unsigned sz) { /* buf - с который мы пишем данные */
    struct pipe *p = fd2pipe(fd, NULL); /* получаем pipe по ФД */
    int read = 0;
    while (read < sz && !(p->wrclose == 1u && p->rd == p->wr)) {
        /* либо прочитали сколько хотели, либо прочитали все что написано, либо pipe закрыт на запись */
        if (p->rd == p->wr) sched_sleep(0); /* если дошли до указателя для зписывания - передаем управление */
        else ((char *) buf)[read++] = p->buf[p->rd++]; /* чтение... */
        if (p->rd == sizeof(p->buf)) p->rd = 0; /* если дошли до конца pipe переносим указатель */
    }
    return read;
}

static int pipe_write(int fd, const void *buf, unsigned sz) { /* аналогично pipe_read */
    struct pipe *p = fd2pipe(fd, NULL);
    int wrote = 0;
    while (wrote < sz && !(p->rdclose == 1u && p->wr == p->rd)) {
        if ((p->wr + 1) % sizeof(buf) == p->rd)
            sched_sleep(0); /* если на следующем шаге дойдем до указателя чтения - передаем управление */
