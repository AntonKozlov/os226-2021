...
int first_unallocated_bit(unsigned long bits) {
    return ffsl(~bits) - 1;
}

static int bitmap_alloc(unsigned long *bitmap, size_t size) {
    for (size_t i = 0; i < size; i++) {
        if (bitmap[i] == ~0ul) continue; //no free bits
        int unallocated_bit_pos = first_unallocated_bit(bitmap[i]);
        bitmap[i] |= 1ul << unallocated_bit_pos;
        return i * (sizeof(unsigned long) * CHAR_BIT) + unallocated_bit_pos;
    }
    return -1;
}
