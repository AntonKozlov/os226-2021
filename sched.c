...
static int do_exec(const char *path, char *argv[]) {
    ...
    Elf64_Ehdr *ehdr = (Elf64_Ehdr *) rawelf;
    Elf64_Phdr *phdr = (Elf64_Phdr *) (rawelf + ehdr->e_phoff);

    void *brk = current->vm.brk * PAGE_SIZE + USER_START - PAGE_SIZE + 1;
    for (int i = 0; i < ehdr->e_phnum; i++) {
        if (phdr[i].p_type != PT_LOAD) continue;
        long a = phdr[i].p_vaddr + phdr[i].p_memsz;
        brk = (void *) a > brk ? (void *) a : brk;
    }
    vmctx_brk(&current->vm, brk);
    vmctx_apply(&current->vm);

    for (int i = 0; i < ehdr->e_phnum; i++) {
        if (phdr[i].p_type != PT_LOAD) continue;
        memcpy((void *) phdr[i].p_vaddr, rawelf + phdr[i].p_offset, phdr[i].p_filesz);
        int prot = (phdr[i].p_flags & PF_W ? PROT_WRITE : 0) |
                   (phdr[i].p_flags & PF_R ? PROT_READ : 0) |
                   (phdr[i].p_flags & PF_X ? PROT_EXEC : 0);
        if (vmprotect((void *) phdr[i].p_vaddr, phdr[i].p_memsz, prot))
            return 1;
    }
