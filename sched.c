...
static int bitmap_alloc(unsigned long *bitmap, size_t size) {
    int bit = 0;
    for (bit = 0; (bitmap[bit / sizeof(unsigned long)] & (1 << (bit % sizeof(unsigned long)))) != 0
                  && bit < size / sizeof(unsigned long); bit++);
    if (bit == size / sizeof(unsigned long))
        return -1;
    ...
    for (int i = 0; i < USER_PAGES; i++) {
        if (vm->map[i] != -1) {
            ...
            mmap(USER_START + i * PAGE_SIZE, PAGE_SIZE, PROT_READ | PROT_WRITE,
                 MAP_SHARED, memfd,vm->map[i] * PAGE_SIZE);
        }
    }
}
